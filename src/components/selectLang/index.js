...
const App = () => {
  ...
  useEffect(() => {
    const timeOutId = setTimeout(() => setTextToTransate(query), 1000);
  ...
  return (
    <div className="App bg-light h-[100vh]">
      <Navbar />
      {/* Translation */}
      <div className="translation w-11/12 md:w-10/12 lg:w-9/12 mx-auto relative flex mt-24">
        <div className="translate-box-left relative w-1/2 h-96 rounded-xl p-5 shadow-md shadow-ligth bg-white">
          <Listbox
            value={translateFrom}
            onChange={setTranslateFrom}
            className="w-3/4 bg-red-200"
          >
            {({ open }) => (
              <>
                <div className="mt-1 relative w-3/4">
                  <Listbox.Button className="relative w-full bg-white border-2 border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    <span className="flex items-center">
                      <img
                        src={translateFrom.avatar}
                        alt=""
                        className="flex-shrink-0 h-6 w-6 rounded-sm"
                      />
        ...
        <div className="translate-box-right relative w-1/2 h-96 p-5 rounded-xl shadow-md shadow-ligth bg-white">
          {translator}
          <div className="action-btn flex items-center absolute bottom-5 right-5">
            <button className="flex items-center justify-center w-9 h-9 mx-1 rounded-md">
              <img src={Volume} alt="volume-icon" className="w-7" />
